...
namespace blib {
	...
	BLIB_EXPORT TriangleArray blib::generateSphere(float radius, size_t polarSegments, size_t azimuthSegments)
	{
	...
	BLIB_EXPORT void blib::TriangleMesh::recomputeNormals()
	{
		for (auto & f : faces) {
	...
	BLIB_EXPORT AABB TriangleMesh::bounds() const
	{
		AABB b;
		for (auto & v : vertices) {
	...
	BLIB_EXPORT std::unique_ptr<Geometry> TriangleMesh::normalized(bool keepAspectRatio /*= true*/) const
	{
		auto b = bounds();
	...
	BLIB_EXPORT std::unique_ptr<Geometry> TriangleMesh::transformed(const Transform & t) const
	{
	...
	BLIB_EXPORT TriangleArray TriangleMesh::getTriangleArray() const
	{
