...
__global__ void ___generateTriangles(
	...
	const float3 targetVoxelSize = make_float3(2.0f / params.res.x, 2.0f / params.res.y, 2.0f / params.res.z);
...
)
{
...

	{
		...
		___generateTriangles<< <grid, TRIANGLE_THREADS >>>(
			params, input, compacted, vertCountScan, activeN, static_cast<blib::CUDA_VBO::DefaultAttrib *>(cudaVBO.getPtr())
			);
